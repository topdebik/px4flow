...
class PX4Flow:
    ...
    def update(self):
        ...
        data = list(
            map(
                lambda x: hex(x)[2:],
                self.BUS.read_i2c_block_data(self.ADDRESS, 0x00, 22),
            )
        )
    ...
    def update_integral(self):
        ...
        data = list(
            map(
                lambda x: hex(x)[2:],
                self.BUS.read_i2c_block_data(self.ADDRESS, 0x00, 27),
            )
        )
        ...
        integration_timespan = shift32(
            int(data[15] + data[14] + data[13] + data[12], 16)
        )
